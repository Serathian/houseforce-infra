...
if [ ! -f "${REPO_DIR}/.env" ]; then
  ...
  read -p "Enter Primary Domain [houseforce.es]: " DOMAIN; DOMAIN=${DOMAIN:-houseforce.es}
  ...
  echo "=== Managed Database Configuration ==="
  read -p "Enter DO Managed Database Host: " DATABASE_HOST
  read -p "Enter DO Managed Database Port [25060]: " DATABASE_PORT; DATABASE_PORT=${DATABASE_PORT:-25060}
  read -p "Enter Database Name: " DATABASE_NAME
  read -p "Enter Database User: " DATABASE_USERNAME
  read -s -p "Enter Database Password: " DATABASE_PASSWORD; echo
  
  echo "=== Google SSO Configuration ==="
  read -p "Enter Google Client ID: " GOOGLE_CLIENT_ID
  read -s -p "Enter Google Client Secret: " GOOGLE_CLIENT_SECRET; echo

  echo "=== DO Spaces Configuration ==="
  read -s -p "Enter DO Spaces Access Key: " DO_SPACE_ACCESS_KEY; echo
  read -s -p "Enter DO Spaces Secret Key: " DO_SPACE_SECRET_KEY; echo
  read -p "Enter DO Spaces Endpoint: " DO_SPACE_ENDPOINT
  read -p "Enter DO Spaces Bucket: " DO_SPACE_BUCKET
...
# Prod deployment uses only the base docker-compose
